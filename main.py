...
while running:
    ...
    p1_hbox_top = pygame.Rect(
        paddle_x,
        paddle_y,
        paddle_size_x,
        paddle_size_y / 3
    )
    p1_hbox_mid = pygame.Rect(
        paddle_x,
        paddle_y + paddle_size_y / 3,
        paddle_size_x,
        paddle_size_y / 3
    )
    p1_hbox_bot = pygame.Rect(
        paddle_x,
        paddle_y + paddle_size_y * 2 / 3,
        paddle_size_x,
        paddle_size_y / 3
    )
    if (
        ball_x_vel < 0
        and (
            b_hbox.colliderect(p1_hbox_top)
            or b_hbox.colliderect(p1_hbox_mid)
            or b_hbox.colliderect(p1_hbox_bot)
        )
    ):
        ball_x_vel *= -1
        ball_x = paddle_x + paddle_size_x

        if b_hbox.colliderect(p1_hbox_top):
            ball_y_vel -= random.randint(0, paddle_bounce)

        elif b_hbox.colliderect(p1_hbox_bot):
            ball_y_vel += random.randint(0, paddle_bounce)
